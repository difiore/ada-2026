...
> **NOTE:** The following code block cannot be rendered to show you the output because it requires an interactive ***R*** environment for several functions (e.g., `drive_auth()`). It also uses the native pipe operator (`|>`), which we have not covered yet, but is discussed in detail in [**Module 10**](#module-10).
