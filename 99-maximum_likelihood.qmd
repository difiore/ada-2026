...
An alternative for MLE is the `maxLik()` function from the {maxLik} package. To use function, we again first define our own log likelihood function, but this time we do not want an negative log likelihood, so we do not multiply it by minus 1. This is because the first argument to `maxLik()` is a log likelihood function, not a **negative** log likelihood function. Our function should also have only one argument... a **vector** of parameter values... rather than separate arguments for each parameter.
