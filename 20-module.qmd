...
[{dunn.test}](https://cran.r-project.org/web/packages/dunn.test/dunn.test.pdf), [{conover.test}](https://cran.r-project.org/web/packages/conover.test/conover.test.pdf),
[{datawizard}](https://cran.r-project.org/web/packages/datawizard/datawizard.pdf),
[{sjPlot}](https://cran.r-project.org/web/packages/sjPlot/sjPlot.pdf),
[{emmeans}](https://cran.r-project.org/web/packages/emmeans/emmeans.pdf), and [{permuco}](https://cran.r-project.org/web/packages/permuco/permuco.pdf)
...
Note that, for these data, the p value estimated through simulation-based inference is very similar to that calculated parameterically based on mathematical theory. This is not always the case!
...
# converts age to a factor and order the age levels so that they are in ascending chronological order
# several ANOVA functions require that categorical variables are represented as factors rather than as strings
...
d <- d |>
  ...
  # and coerce age back to being a factor
  mutate(age = factor(age, level = c("I", "J", "S", "A")))

m.aov <- aov(femur_length ~ age, data = d)
posthoc <- TukeyHSD(m.aov,
                    which = "age",
                    ordered = TRUE,
                    conf.level = 0.95)
...
# the arguments are a vector of data, a vector with the grouping factor, and the multiple test correction method...
pairwise.wilcox.test(d$femur_length, d$age,
  p.adjust.method="bonferroni")

# `dunn.test()` and `conover.test()` include another argument, `kw=`, which is whether or not to output KW results
library(dunn.test)
...
Here, 81% of the variance in **femur length** is accounted for by variance in the grouping variable **age**.
...
```{r}
m <- lm(femur_length ~ age, data = d)
effectsize(m)
```

> **NOTE**: The {datawizard} package provides the function `standardize()`, which will also take a model and refit it using standardized parameters and return a refitted model object.

```{r}
#| message: false
library(datawizard)
standardize(m)
detach(package:datawizard)
```
...
When we run `effectsize()` on a model object resulting from a multiple factor ANOVA, the function, by default, returns partial $\eta^2$ values, which is the proportion of the variance in the response variable that is accounted by each categorical predictor *when controlling for the other predictors*.
...
To examine whether there is an interaction effect between our multiple categorical predicto variables, we would modify our model formula a bit using the colon operator (`:`) to specify a particular interaction term...
...
Note that when we run `summary()` on the results of the `lm()` function (as opposed to running it on the results of the `aov()` function), we are estimating eight $\beta$ coefficients (equivalent to the number of groups we have). Here, $\beta_0$, the intercept, is the mean femur length for the base level (in this case, "adult females"). Then we have coefficients showing how the different factor combination groups would differ from that base level (e.g., adult males have mean femur lengths 1.716 greater than adult females, etc).

### Visualizing Interactions

We can use the function `interaction.plot()` to visualize interactions among categorical variables in our raw data...

```{r}
#| fig-height: 4
interaction.plot(
  x.factor = factor(d$age, level = c("I", "J", "S", "A")),
  xlab = "Age",
  trace.factor = d$sex,
  trace.label = "Sex",
...
Here, it appears like there is indeed a significant main effect of each term (within several the adult and subadult categories, mean femur length differs for males and females), as well as an interaction between our two categorical variables (the same does not appear to be true for infants and juveniles).

We can also use either the {sjPlot} or {emmeans} packages to look at differences in **Mean Femur Length** predicted by our multiple ANOVA model. These plot the estimated marginal means, or the average predicted scores scores for each independent variable at a given level of the other independent variable...
...
Note that these plots show the model-predicted mean value for the response variable **femur length** for each combination of the two categorical predictors (**age** and **sex**).

- Differences between points at a given **age** indicate an effect of **sex** within that particular **age** category.
- Non-parallel lines in the interaction plot (like we have here) indicate an interaction between predictors. If one **sex** has a very different pattern across **age** than the other, then that is evidence that the interaction term is meaningful (and, in this case, would suggest that developmental trajectories differ by **sex**). Parallel lines would suggest no interaction (and, thus, that developmental trajectories are similar in the two sexes).
- Crossing lines in a marginal means plot would indicate a strong interaction among predictor terms, e.g., where the ranking of **femur length** by **sex** actually reverses with **age** class.

We will return to comparing alternative ANOVA models to one another (e.g., our model with and without interaction terms) and to post-hoc tests of which group mean differences are significant when we get into model selection in [**Module 22**](#module-22).
...
It is important to recognize that the **order** in which our factors are entered into a linear model using multiple categorical predictors can result in different values for the entries in our ANOVA table, while the estimation of our regression coefficients is identical regardless. Take a look at this example, where the variables **age** and **sex** are entered into our ANOVA formula in different orders:
