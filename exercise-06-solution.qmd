...
hr_summary <- d |>
  ...
  summarize(mean = mean(kernel95),
            sd = sd(kernel95),
            n = n(),
            # note: do not use nrow(d) as that would be the entire dataset
            se = sd/sqrt(n)
  )
hr_summary
```
...
```{r}
# using do()*... note that entire do()* code is wrapped in parentheses before pull()
perm_diff <- (do(n_perm) *
  d |>
    mutate(sex=sample(sex)) |>
    summarize(diff =	mean(kernel95[sex == "M"]) -
...
# using replicate()... note the {} wrapped around the code within replicate() to perform in each replicate
