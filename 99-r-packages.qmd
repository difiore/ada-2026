...
> **NOTE:** This method differs from **Method 1** in that it creates a `man/` directory and sets up your package development repository as a ***git*** repository from the onset.
...
Imports:
  ggplot2
````
Also, because I tend to use this function inside of a function from {manipulate} (see [**Module 17**](#module-17)), I also run the following to add a "Suggests" block to the DESCRIPTION file:
...
You could also run this from the terminal, in which case we would need to be in the parent directory for our package and we would need to reference the parent directory as the argument for the command (note the `/` at the end of) `powerplot/`...

```
R CMD check powerplot/
```
...
Now try out your new function! First, use `library(powerplot)` to load in your library. Then, here is what you should see in the "Help" tab when you type `?power.plot` at console the line.
...
> **NOTE:** The function `.libPaths()` will list the full paths to any library directories where ***R*** has installed packages, and the first element in this vector will be the directory to which new packages are installed by default.
...
- Put your vignette ".qmd" file in that directory.
...
The command `use_vignette()` from {usethis} will do all of this for us and give us a template ".qmd" file we can edit for our vignette...

```{r}
#| eval: false
usethis::use_vignette("powerplot.qmd", title = "Power Plot Vignette")
```

Once you have your ".qmd" file completed, can render your vignette by running `devtools::build()` again...
...
The resulting rendered ".html" version of your vignette will be placed into the `inst/doc` folder inside the package directory and the vignette will be bundled into your ".tar.gz" package file.
...
Then, run `document()`, `build()`, and `check()` again as above to create the package anew with the data included in it, and run `install()` and `library()` to load it into your workspace. Voíla! You can confirm that your dataset loaded by running the following...
...
### Build the Package and Vignettes {.unnumbered}
