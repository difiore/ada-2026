...
```{r}
#| include: false
options(dplyr.summarize.inform = FALSE) # to suppress extraneous messages
```
...
s <- {do(reps) * sample_n(group_by(d, decade), n, replace=FALSE)} |>
  # note that the entire do()* loop code needs to be wrapped in braces (or parentheses) in order to introduce the variable ".index" that we need to then pass as an argument to `group_by()` before pull()
  group_by(decade, .index) |>
  summarize(avg_runtimeMinutes = mean(runtimeMinutes, na.rm = TRUE), sd_runtimeMinutes = sd(runtimeMinutes, na.rm = TRUE))
...
```{r}
#| message: false
pop_means <- dplyr::summarize(survivors, mean(height), mean(weight),  mean(age), mean(zombies_killed), mean(years_of_education)) # {dplyr} rocks!

pop_sds <- dplyr::summarize(survivors, sd_pop(height), sd_pop(weight), sd_pop(age), sd_pop(zombies_killed), sd_pop(years_of_education)) # {dplyr} rocks!

# or, using `sdpop()` from {radiant}
# pop_SDs <- dplyr::summarize(survivors,
#   sdpop(height),
#   sdpop(weight),
#   sdpop(age),
...
# ALTERNATIVELY...
#   we can use new dplyr::summarize(across()) construction
...
samp_1_means <- s |> dplyr::summarize(across(
  .cols = c(height, weight, age, zombies_killed, years_of_education),
  .fns = ~ mean(.)))

samp_1_SDs <- s |> dplyr::summarize(across(
  .cols = c(height, weight, age, zombies_killed, years_of_education),
  .fns = ~ sd(.)))

samp_1_SEs <- s |> dplyr::summarize(across(
...
samp_dist_means <- samp_dist |>  dplyr::summarize(across(
  .cols = everything(),
  .fns = ~ mean(.)))

samp_dist_SDs <- samp_dist |>  dplyr::summarize(across(
  .cols = everything(),
  .fns = ~ sd(.)))
...
```{r}
samp_SE_means <- samp_SEs |>  dplyr::summarize(across(
  .cols = everything(),
  .fns = ~ mean(.)))
...
samp_dist_CI <- dplyr::summarize(samp_dist, across(
  .cols = everything(),
