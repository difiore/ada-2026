...
> **NOTE:** I usually install packages to the my personal *user* library rather than at the *system* level (and, depending on permissions about software installation on your device, you may not even be able to install to a system-level library). For a user-specific installation, your library is typically located at "~/Library/R/\<architecture\>/\<R-version\>/library", where `<architecture>` refers to your computer's processor type (e.g., "arm64") and `<R-version>` (e.g., "4.5.2") is the version number of your ***R*** installation. The system-wide library is located at "/Library/Frameworks/R.framework/Versions/\<R-version\>/Resources/library". Installing to system-wide library makes packages accessible to all users on the system, but it typically requires administrator privileges to install packages to this location.
...
- Select a package to install. An install location should be selected by default. Typically, this will be to a personal *user* library, which on a Windows machine is likely to be at "C:\\Users\\\<username\>\\AppData\\Local\\R\\win-library\\\<R-version\>" (where `<R-version>` is the ***R*** major.minor version number, e.g., "4.5"). If you have administrator privileges, then packages may be installed by default at the *system* level (inside of the **library** folder within your machine's ***R*** installation, usually at "C:\\Program Files\\R\\\<R-version\>\\library" (where `<R-version>` is the full ***R*** version number, e.g., "4.5.2").

Any missing dependencies should be installed by default.

#### Using the ***R*** console prompt {.unnumbered}

- Use the function `install.packages("<package name>")`, where you include, between the quotes, the name of the package you want to install. This command installs the package, by default, to a personal *user* library, though this can be changed by providing a full path to install to using the `lib="<path>` argument. Other arguments for this function can be set to specify the repository to download from, etc.
...
- To remove installed packages from your ***R*** installation (i.e., to delete them from the *user* or *system* location where they have been installed), you can use the function `remove.packages()` or click the small "x" to the right of the package name in the ***RStudio*** packages tab.
