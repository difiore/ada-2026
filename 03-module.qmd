...
I usually install packages to the my personal *user* library rather than at the *system* level (and, depending on permissions about software installation on your device, you may not even be able to install to a system-level library). For a user-specific installation, your library is typically located at "~/Library/R/<architecture>/<R-version>/library", where `<architecture>` refers to the processor type (e.g., "arm64") and `<R-version>` (e.g., `4.5.2`) is the version number of your ***R*** installation. The system-wide library is located at "/Library/Frameworks/R.framework/Versions/<R-version>/Resources/library" and is used to make packages accessible to all users on the system. It often requires administrator privileges to install packages to the system-wide library.
...
- Select a package to install. An install location should be selected by default. Typically, this will be to a personal *user* library, which on a Windows machine is likely to be at "C:\\Users\\YourUsername\\AppData\\Local\\R\\win-library\\x.y" (where `x.y` is the ***R*** major.minor version number, e.g., "4.5"). If you have administrator privileges, then packages may be installed by default at the *system* level (inside of the **library** folder within your machine's ***R*** installation, usually at "C:\\Program Files\\R\\R-x.y.z\\library" (where `x.y.z` is the full ***R*** version number).
