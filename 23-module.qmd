...
$$g_i = logit(\pi_i) = \ln[\frac{\pi_i}{(1-\pi_i)}] = \beta_0 + \beta_1x_i$$
...
$$expit(g_i) = \frac{e^{g_i}}{1+e^{g_i}} = \frac{1}{1+e^{-g_i}}=\pi_i$$
...
d$rank <- factor(d$rank, levels = c(1, 2, 3, 4))
```
...
p <- ggplot(data = d, aes(x = gpa, y = admit)) +
  geom_point(color="green") +
  xlab("GPA") +
  ylab("Admit") +
  ggtitle("Pr(Y) versus GPA")
q <- ggplot(data = d, aes(x = gpa, y = admit)) +
  geom_point(color="blue") +
  xlab("GRE") +
  ylab("Admit") +
  ggtitle("Pr(Y) versus GRE")
library(cowplot)
...
#| fig-height: 4
#| message: false
x <- seq(from = 200, to = 800, by = 1)
log_odds <- predict(glm, newdata=data.frame(gre=x))
# this function will predict the log(odds)...
# running the `exp()` function, i.e., `exp(log_odds)` will yield the actual odds!
odds <- exp(log_odds)

# if we add the argument `type="response"`, the `predict()` function will return the expected response on the scale of the Y variable, i.e., Pr(Y)
y <- predict(glm, newdata = data.frame(gre = x), type = "response")

# plotting our original data and the model
res <- tibble(
  gre = x,
  log_odds = log_odds,
  odds = odds,
...
head(res)
tail(res)
p <- ggplot(data = d, aes(x = gre, y = admit)) +
  geom_point(alpha = 0.5) +
  xlab("GRE Score") + ylab("Pr(admit)") +
  ggtitle("Pr(admit) versus GRE Score") +
  geom_line(data = res, aes(x = gre, y = pr_admit))
p
...
results$lower <- results$estimate -
  qnorm(1 - alpha / 2) * results$std.error
results$upper <- results$estimate +
  qnorm(1 - alpha / 2) * results$std.error
(CI <- results |> select(c("term", "lower", "upper")))
```
...
# exponentiate to convert to the odds scale instead of log(odds) scale
(odds <- exp(log_odds))
(CI_odds <- exp(CI)) # exponentiate to convert to odds scale
```
...
#| code-fold: true
#| fig-width: 4
x <- seq(from = 2, to = 4, length.out = 100)
...
# IMPORTANT: Using type="response" returns predictions on the scale of the Y variable, in this case Pr(admit). Using the default for the `type=` argument would return a prediction on the logit scale, i.e., the log(odds), which equals log(Pr(admit)/(1-Pr(admit)))
head(res)
p <- ggplot(data = d, aes(x = gpa, y = admit)) +
  geom_point() +
  xlab("GPA") +
  ylab("Pr(admit)") +
  ggtitle("Pr(admit) versus GPA") +
  geom_line(data = res, aes(x = gpa, y = pr_admit))
p
```
...
#| message: false
alpha <- 0.05
y <- predict(glm, newdata = data.frame(gpa = x), type = "response", se.fit = TRUE)
res <- tibble(
  gpa = x,
  pr_admit = y$fit,
  se = y$se.fit
...
res$lower <- res$pr_admit -
  qnorm(1 - alpha / 2) * res$se
res$upper <- res$pr_admit +
  qnorm(1 - alpha / 2) * res$se
head(res)

# plot...
# note we start by plotting the model data (res) b/c we are hand-plotting the SE ribbon
p <- ggplot(data = res, aes(x = gpa, y = pr_admit)) +
  geom_line() +
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.2) +
  # and then we add in points
...
# or, alternatively...
# note here we start with plotting the raw data
p <- ggplot(data = d, aes(x = gpa, y = admit)) +
  geom_point(alpha = 0.5) +
  xlab("GPA") +
  ylab("Pr(admit)") +
  ggtitle("Pr(admit) versus GPA") +
  # and then add in the fitted line via glm
  geom_smooth(method = "glm", method.args = list(family = binomial)) 
p
```
...
(p <- pchisq(q = 0, df = 0, lower.tail = FALSE))
...
```{r}
# compare each 2 predictor model to 3 predictor model
...
glmGGR_no <- glm(data = d, admit ~ rank + gpa + gre, family = "binomial")
glmGGR_all <- glm(data = d, admit ~ rank * gpa * gre, family = "binomial")
lrtest(glmGGR_no, glmGGR_all)
...
d$rank <- factor(d$rank, levels = c("low", "medium", "high"))
summary(d)
# first, some exploratory visualization
...
# IMPORTANT: Using the argument `type="response"` makes our prediction be in units of our actual Y variable, success, rather than log(success)
...
lrtest(reduced, proposed)
# or, by using the `anova()` function
...
(p <- 1 - pchisq(x2, df = 1))
# or, calculating the model deviances by hand using the `logLik()` function (which returns the log-likelihood of the model) and then taking the difference in deviance
...
library(sjPlot)
library(effects)
plot_model(glm3, type = "eff", term = "age") +
  geom_point(data = d, aes(x = age, y = success))
plot_model(glm3, type = "eff", term = "rank")
