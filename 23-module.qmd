...
res$lower <- res$pr_admit - qnorm(1 - alpha / 2) * res$se
res$upper <- res$pr_admit + qnorm(1 - alpha / 2) * res$se
head(res)

# plot... note we start with model b/c we are hand-plotting the SE ribbon
...
# or, alternatively
# note we start with raw data
p <- ggplot(data = d, aes(x = gpa, y = admit)) +
  geom_point(alpha = 0.5) +
  xlab("GPA") +
  ylab("Pr(admit)") +
  ggtitle("Pr(admit) versus GPA") +
  geom_smooth(method = "glm", method.args = list(family = binomial)) # and add in smoothed line via gml
p
```
