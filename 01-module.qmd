...
#| echo: false
#| out-width: "90%"
knitr::include_graphics("img/R-console-macOS.png")
```
...
- The assignment operator `<-` can be used to assign a value, the results of an operation, or specific code to an *object* (e.g., a variable, a function, a complex data structure).
    ...
    - You can also use `=`, but I prefer (and recommend) to use that **only** to assign values to function *arguments* (more on this later).
...
- Commands and code can also be written in a text file or *script* and sent to the console for execution.
    ...
    - You can include comments in your scripts by prefacing them with `#`. Comments are not interpreted as code.
...
- To remove all objects from your workspace, you can use `rm(list=ls())`.
    ...
    - In this case, you are passing to the `rm()` (or `remove()`) function a list consisting of all the objects in the workspace, which is provided by the `ls()` function.
...
#| echo: false
#| out-width: "90%"
knitr::include_graphics("img/R-terminal-macOS.png")
```
...
#| echo: false
#| out-width: "85%"
knitr::include_graphics("img/R-cmd-windows.png")
```

... and then practice interacting with the software via the command line and console window.

> **NOTE:** To run ***R*** like this from a command prompt, you may need to add the path to the program's executable file to your system's PATH environmental variable. To do this, you need to know the path to the location where ***R*** was installed. On MacOS, this path is usually... "/usr/local/bin/R", which typically is a symbolic link that points to the actual ***R*** framework, located within your system's `/Library/Frameworks/` directory. On Windows, the path to the ***R*** executable is typically "C:\\Program Files\\R\\R-x.x.x", where "x.x.x" is the version number, e.g., "4.5.2".
...
> - Click "New" and add the path to ***R***'s executable `bin/` folder, typically "C:\\Program Files\\R\\R-x.x.x\\bin\\x64" (adjust the version number as needed)
